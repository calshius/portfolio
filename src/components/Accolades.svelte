...
<div class="section-label">Accolades</div>
  ...
  {#each awards as award}
    <div class="item">
      <div class="item-header">
        <span class="badge {badgeClass(award.result)}">{award.result}</span>
      </div>
      <div class="item-body">
        <h3>{award.title}</h3>
        <p class="meta">{award.event}</p>
        <span class="tag">{award.role}</span>
      </div>
    </div>
  {/each}
</div>
